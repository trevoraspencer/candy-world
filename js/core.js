...
(function initCandyCore(root, factory) {
    ...
    function stackMetadataMatches(a,b){return(Number.isFinite(a?.durability)?a.durability:null)===(Number.isFinite(b?.durability)?b.durability:null);}
    function placeOneFromCursor(slots,cursor,index,getMaxStack){const next=cloneSlots(slots),held=cloneSlots([cursor])[0];if(!held||!Number.isInteger(index)||index<0||index>=next.length)return{slots:next,cursor:held,moved:0};const slot=next[index],max=Math.max(1,(getMaxStack||(()=>64))(held.id)||1);if(slot&&(slot.id!==held.id||!stackMetadataMatches(slot,held)||slot.count>=max))return{slots:next,cursor:held,moved:0};if(slot)slot.count++;else next[index]=Object.assign({id:held.id,count:1},Number.isFinite(held.durability)?{durability:held.durability}:{});held.count--;return{slots:next,cursor:held.count?held:null,moved:1};}
    function returnCursorStack(slots,cursor,getMaxStack){const original=cloneSlots(slots),held=cloneSlots([cursor])[0];if(!held)return{ok:false,slots:original,drop:null,moved:0};const returned=transferBetween([held],original,0,getMaxStack);if(returned.moved!==held.count)return{ok:false,slots:original,drop:held,moved:0};return{ok:true,slots:returned.destination,drop:null,moved:returned.moved};}
    function commitCursorStack(slots,cursor,getMaxStack,commitDrop){const returned=returnCursorStack(slots,cursor,getMaxStack);if(returned.ok)return{ok:true,slots:returned.slots,cursor:null,moved:returned.moved,location:'inventory'};const drop=returned.drop;if(drop&&typeof commitDrop==='function'&&commitDrop(cloneSlots([drop])[0])===true)return{ok:true,slots:returned.slots,cursor:null,moved:drop.count,location:'world'};return{ok:false,slots:returned.slots,cursor:drop,moved:0,location:null};}
    ...
    function transferStack(slots,fromIndex,destinationIndices,getMaxStack) {
        ...
        for(const index of destinationIndices){const slot=next[index];if(!slot||slot.id!==source.id||!stackMetadataMatches(slot,source)||slot.count>=max)continue;const amount=Math.min(remaining,max-slot.count);slot.count+=amount;remaining-=amount;moved+=amount;if(!remaining)break;}
    ...
    function distributeStack(slots,cursor,indices,getMaxStack){const next=cloneSlots(slots),held=cursor&&Number.isInteger(cursor.id)&&cursor.count>0?{...cursor}:null;if(!held)return{slots:next,cursor:null,moved:0};const unique=[...new Set(indices||[])].filter(index=>Number.isInteger(index)&&index>=0&&index<next.length),max=Math.max(1,(getMaxStack||(()=>64))(held.id)||1);let moved=0,progress=true;while(held.count>0&&progress){progress=false;for(const index of unique){const slot=next[index];if(slot&&(slot.id!==held.id||!stackMetadataMatches(slot,held)||slot.count>=max))continue;if(slot)slot.count++;else next[index]=Object.assign({id:held.id,count:1},Number.isFinite(held.durability)?{durability:held.durability}:{});held.count--;moved++;progress=true;if(!held.count)break;}}return{slots:next,cursor:held.count?held:null,moved};}
